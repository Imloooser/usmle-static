...
// Lazy-load ScoreResults — only downloaded when user gets a prediction
...
export default function ScorePredictor({ onNavigate }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      Object.entries(scores).forEach(([k, v]) => {
        if (v !== undefined && v !== '') payload[k] = parseFloat(v);
      });
  ...
  const renderGroup = (groupKey, title, icon) => {
    const fields = SCORE_FIELDS.filter(f => f.group === groupKey);
    const expanded = expandedSections[groupKey];
    const filledInGroup = fields.filter(f => scores[f.key] && scores[f.key] !== '').length;
            ...
            <section className="hero" aria-label="Score predictor introduction">
              ...
              <div className="hero-features">
                ...
                <div className="feature">
                  ...
                  <span>No login required &bull; 100% private</span>
                </div>
              </div>
            </section>
